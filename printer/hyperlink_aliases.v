...
fn hyperlink_pattern_aliases() []HyperlinkAlias {
	return [
		...
		alias('kitty', 'kitty-style RFC 8089 scheme (file://) with line number', 'file://{host}{path}#{line}'),
		alias('macvim', 'MacVim scheme (mvim://)', 'mvim://open?url=file://{path}&line={line}&column={column}'),
		prioritized_alias(1, 'none', 'disable hyperlinks', ''),
		alias('textmate', 'TextMate scheme (txmt://)', 'txmt://open?url=file://{path}&line={line}&column={column}'),
		alias('vscode', 'VS Code scheme (vscode://)', 'vscode://file{path}:{line}:{column}'),
		alias('vscode-insiders', 'VS Code Insiders scheme (vscode-insiders://)', 'vscode-insiders://file{path}:{line}:{column}'),
