...
fn binary_hay() []u8 {
	path := os.join_path(@VMODROOT, 'integration', 'data', 'sherlock-nul.txt')
...
fn test_binary_mmap_match_implicit() {
	$if macos {
		return
	}
	dir, mut cmd := setup('binary_mmap_match_implicit')
...
fn test_binary_mmap_match_explicit() {
	$if macos {
		return
	}
	dir, mut cmd := setup('binary_mmap_match_explicit')
	dir.create_bytes('hay', binary_hay())
	cmd.args(['--mmap', '-n', 'Project Gutenberg EBook', 'hay'])

	expected := '1:The Project Gutenberg EBook of A Study In Scarlet, by Arthur Conan Doyle\n'
	eqnice(expected, cmd.stdout())
}

// Test specifically with a pattern that matches near the NUL byte which should
// trigger binary detection with memory maps.
fn test_binary_mmap_match_near_nul() {
	$if macos {
		return
	}
	dir, mut cmd := setup('binary_mmap_match_near_nul')
	dir.create_bytes('hay', binary_hay())
	...
	expected := 'binary file matches (found "\\0" byte around offset 77041)\n'
	eqnice(expected, cmd.stdout())
}

// Test with --count option to ensure full file scanning works with mmap.
fn test_binary_mmap_match_count() {
	$if macos {
		return
	}
	dir, mut cmd := setup('binary_mmap_match_count')
	dir.create_bytes('hay', binary_hay())
...
// byte.
fn test_binary_mmap_match_multiple() {
	$if macos {
		return
	}
	dir, mut cmd := setup('binary_mmap_match_multiple')
	dir.create_bytes('hay', binary_hay())
...
// map.
fn test_binary_mmap_binary_flag() {
	$if macos {
		return
	}
	dir, mut cmd := setup('binary_mmap_binary_flag')
	dir.create_bytes('hay', binary_hay())
	// Use glob pattern.
	cmd.args(['--mmap', '-n', '--binary', 'Heaven', '-g', 'hay'])

	expected := 'hay:1871:"No. Heaven knows what the objects of his studies are. But here we\n'
	eqnice(expected, cmd.stdout())
}

// Test that using -a/--text flag works as expected with mmap.
fn test_binary_mmap_text_flag() {
	$if macos {
		return
	}
	dir, mut cmd := setup('binary_mmap_text_flag')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_after_match1_implicit() {
	dir, mut cmd := setup('binary_after_match1_implicit')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_after_match1_explicit() {
	dir, mut cmd := setup('binary_after_match1_explicit')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_after_match1_stdin() {
	_, mut cmd := setup('binary_after_match1_stdin')
...
fn test_binary_after_match1_implicit_binary() {
	dir, mut cmd := setup('binary_after_match1_implicit_binary')
	dir.create_bytes('hay', binary_hay())
	cmd.args(['--no-mmap', '-n', '--binary', 'Project Gutenberg EBook', '-g', 'hay'])
...
fn test_binary_after_match1_implicit_text() {
	dir, mut cmd := setup('binary_after_match1_implicit_text')
	dir.create_bytes('hay', binary_hay())
	cmd.args(['--no-mmap', '-n', '--text', 'Project Gutenberg EBook', '-g', 'hay'])
...
// detection should be performed.
fn test_binary_after_match1_explicit_text() {
	dir, mut cmd := setup('binary_after_match1_explicit_text')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_after_match1_implicit_path() {
	dir, mut cmd := setup('binary_after_match1_implicit_path')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_after_match1_implicit_quiet() {
	dir, mut cmd := setup('binary_after_match1_implicit_quiet')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_after_match1_implicit_count() {
	dir, mut cmd := setup('binary_after_match1_implicit_count')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_after_match1_implicit_count_binary() {
	dir, mut cmd := setup('binary_after_match1_implicit_count_binary')
	dir.create_bytes('hay', binary_hay())
	cmd.args(['--no-mmap', '-c', '--binary', 'Project Gutenberg EBook', '-g', 'hay'])
	eqnice('hay:1\n', cmd.stdout())
}
...
// reported.
fn test_binary_after_match1_explicit_count() {
	dir, mut cmd := setup('binary_after_match1_explicit_count')
	dir.create_bytes('hay', binary_hay())
...
// after a NUL byte.
fn test_binary_before_match1_implicit() {
	dir, mut cmd := setup('binary_before_match1_implicit')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_before_match1_explicit() {
	dir, mut cmd := setup('binary_before_match1_explicit')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_before_match1_implicit_binary() {
	dir, mut cmd := setup('binary_before_match1_implicit_binary')
	dir.create_bytes('hay', binary_hay())
	cmd.args(['--no-mmap', '-n', '--binary', 'Heaven', '-g', 'hay'])

	expected := 'hay: binary file matches (found "\\0" byte around offset 77041)\n'
	eqnice(expected, cmd.stdout())
}

// Like before_match1_implicit, but enables -a/--text, so no binary
// detection should be performed.
fn test_binary_before_match1_implicit_text() {
	dir, mut cmd := setup('binary_before_match1_implicit_text')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_before_match2_implicit() {
	dir, mut cmd := setup('binary_before_match2_implicit')
	dir.create_bytes('hay', binary_hay())
...
fn test_binary_before_match2_explicit() {
	dir, mut cmd := setup('binary_before_match2_explicit')
	dir.create_bytes('hay', binary_hay())
...
// Like before_match1_implicit, but enables -a/--text, so no binary
// detection should be performed.
fn test_binary_before_match2_implicit_text() {
	dir, mut cmd := setup('binary_before_match2_implicit_text')
	dir.create_bytes('hay', binary_hay())
	cmd.args(['--no-mmap', '-n', '--text', 'a medical student', '-g', 'hay'])

	expected := 'hay:1867:"And yet you say he is not a medical student?"\n'
	eqnice(expected, cmd.stdout())
}

// See: https://github.com/BurntSushi/ripgrep/issues/3131
fn test_binary_matching_files_inconsistent_with_count() {
	dir, _ := setup('binary_matching_files_inconsistent_with_count')
	mut builder := strings.new_builder(8 + (150_000 * 13) + 1)
	builder.write_string('cat here\n')
	for _ in 0 .. 150_000 {
		builder.write_string('padding line\n')
	}
	builder.write_u8(0)
	file1 := builder.str()
	...
	cmd2.args(['--sort=path', '-c', 'cat'])
	eqnice('file2.txt:1\n', cmd2.stdout())

	mut cmd3 := dir.command()
	cmd3.args(['--sort=path', '-c', 'cat', '--binary'])
	eqnice('file1.txt:1\nfile2.txt:1\n', cmd3.stdout())

	mut cmd4 := dir.command()
	cmd4.args(['--sort=path', '-c', 'cat', '--text'])
	eqnice('file1.txt:1\nfile2.txt:1\n', cmd4.stdout())
}
