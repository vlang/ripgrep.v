...
const stdin_read_capacity = 8 * (1 << 10)
...
// V-specific: Rust's `stdin.lock()` path is buffered. Limit individual stdin
// reads so line-buffer binary detection observes the same stream boundaries.
struct StdinReader {
mut:
	file os.File
}

fn StdinReader.new(file os.File) StdinReader {
	return StdinReader{
		file: file
	}
}

fn (mut rdr StdinReader) read(mut buf []u8) !int {
	if buf.len <= stdin_read_capacity {
		return rdr.file.read(mut buf)!
	}
...
pub struct SearchResult implements IClone {
	has_match bool
mut:
	stats_value printer.Stats
	has_stats   bool
}

fn SearchResult.new(has_match bool, stats ?printer.Stats) SearchResult {
	mut result := SearchResult{
		has_match: has_match
	}
	if stats_value := stats {
		result.stats_value = stats_value
...
fn (mut matcher_ PatternMatcher) drop() {
	match matcher_.kind {
		.rust_regex { matcher_.regex.drop() }
		.pcre2 { matcher_.pcre2.drop() }
	}
}
...
fn (mut worker SearchWorker[W]) search_preprocessor(path string) !SearchResult {
	...
	result := worker.search_command_reader(path.clone(), &rdr) or {
		...
		return error("preprocessor command failed: '${bin} ${path}': ${search_err.msg()}")
	}
	rdr.close() or {
...
fn (mut worker SearchWorker[W]) search_decompression_reader(path string, rdr &cli.DecompressionReader) !SearchResult {
	...
	pub fn (mut worker SearchWorker[W]) search_path(path string) !SearchResult {
		return match worker.matcher.kind {
			.rust_regex {
				worker.search_path_with_regex_matcher(&worker.matcher.regex,
					printer.PrinterMatcher.rust_regex(&worker.matcher.regex), path.clone())
			}
			.pcre2 {
				worker.search_path_with_pcre2_matcher(&worker.matcher.pcre2,
					printer.PrinterMatcher.pcre2(&worker.matcher.pcre2), path.clone())
			}
		}
	}
	...
	pub fn (mut worker SearchWorker[W]) search_reader(path string, mut rdr io.Reader) !SearchResult {
		return match worker.matcher.kind {
			.rust_regex {
				worker.search_reader_with_regex_matcher(&worker.matcher.regex,
					printer.PrinterMatcher.rust_regex(&worker.matcher.regex), path.clone(), mut rdr)
			}
			.pcre2 {
				worker.search_reader_with_pcre2_matcher(&worker.matcher.pcre2,
					printer.PrinterMatcher.pcre2(&worker.matcher.pcre2), path.clone(), mut rdr)
			}
		}
	}
...
fn normalize_searcher_error(err IError) IError {
	if err.code() == 0 && err.msg() == '' {
		return cli.broken_pipe_error()
	}
	return err
}
